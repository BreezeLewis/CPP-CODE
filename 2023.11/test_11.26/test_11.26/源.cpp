...
>myuncompress soucefile [targetfile]
*/
...
void TransWrite(char* _01Str, int _01StrLen, FILE* pfile)
{
	...
	for (i = 7, bytenum = 0; i < _01StrLen; i += 8)
	{
		...
		Set_bit(BitCode[bytenum], 1, _01Str[i] - '0');
		Set_bit(BitCode[bytenum], 2, _01Str[i - 1] - '0');
		Set_bit(BitCode[bytenum], 3, _01Str[i - 2] - '0');
		Set_bit(BitCode[bytenum], 4, _01Str[i - 3] - '0');
		Set_bit(BitCode[bytenum], 5, _01Str[i - 4] - '0');
		Set_bit(BitCode[bytenum], 6, _01Str[i - 5] - '0');
		Set_bit(BitCode[bytenum], 7, _01Str[i - 6] - '0');
		Set_bit(BitCode[bytenum], 8, _01Str[i - 7] - '0');
		++bytenum;
	}
	BitCode[bytenum] = '\0';
